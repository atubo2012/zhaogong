...
Page({
    ...
    onLoad: function (option) {
        ...
        if (typeof(option.reqId) !== "undefined") {
                ...
                success: function (res) {
                    ...
                    if ("0" === ret) {
                        ...
                        that.setData({
                            ...
                            'rdata.cost': ret.cost,
                            'biz_type':ret.biz_type,//不雅：库里的biz_type就是cc中的charging_type
                            '_ccs': ret.ct.ccs,//页面使用_ccs来显示或隐藏CC，让页面代码更加简洁
                        ...
                        //SOP：根据业务品类渲染商品选择器和图片cc_bizcart
                        cc_bizcart._initBizCart(ret.biz_type);
                        ...
                        if (that.data.rdata.stat === 'bs_wait') {
                            ...
                            that.setData({
                                ...
                                'cc_rdata_pay.body': 'BT:'+ret.biz_type,
                        ...
                        //更新供应商需要的按钮
                        if(that.data.rdata.supplier_id.indexOf(app.globalData.userInfo.openId)>=0){
        ...
        } else if(option.biz_type){
            /**
             * 新增模式
             *
             * 全局参数中取得从前一个点击获得的商品类别信息
             * 根据该商品类别信息
             */
            let p = app.globalData.param;//在param在index.js中被设置
            cc_bizcart._initBizCart(option.biz_type);

            ut.debug('进入新增模式，隐藏详情页面',cc_bizcart);
            ...
            ut.checkSession(null, app, that, () => {
                that.setData({
                    ...
                    //动态商品有关信息
                    'biz_type'          : option.biz_type,//从页面参数中传过来
                    'rdata.pics_list'   : [],//将要传到后端的图片，仅在ccs.uploadpic==true时才有效
                    '_ccs'              : cf.default_ccs,//默认的
                    'rdata.supplier_id' :p.supplier_id,
                    'rdata.ct'          :Object.assign(p,{
                        url:'order-edit',
                        ccs:cf.default_ccs,//默认的表单数据
                    }),
                    postTitle           :p.post_title,
                    backGroundImagUrl   :p.pics_list[0]
                });
                //console.log('========================================that.data.rdata',that.data);

                //初始化海报控件，必须是在postTitle和backGroundImagUrl获取到之后，再初始化控件，否则发给到后端的请求中将缺失参数。
                let cc_poster = that.selectComponent("#cc_poster");
                this.setData({  //cc_poster需要设置userInfo属性
    ...
    onSubmit: function (e) {
        ...
        rdata.ct = that.data.rdata.ct;

        //计算平台佣金cop和代理佣金coa
        //平台佣金=子类的平台佣率*子类的小计金额
        //代理佣金=子类的代理佣率*子类的小计金额
        //两个佣金都保存为订单一级字段，便于查看
        //算法：遍历goods过程中，累加cop和coa；遍历完成后，将cop和coa设置订单字段中
        let cop=0;let coa = 0;
        that.data.rdata.cart.goods.map((item, index, arr) => {
            cop += item.roc4p ? Number(item.subtotal)*Number(item.roc4p)/100:0;
            coa += (item.roc4a&&rdata.uid!=="") ? Number(item.subtotal)*Number(item.roc4a)/100:0;
            console.log('累加佣金',item.subtotal,item.roc4p,item.roc4a);
        });
        rdata.cop = cop;
        rdata.coa = coa;
        ...
        if (goOne) {
            console.log('order-edit rdata', rdata);
            wx.request({
                        ...
                        wx.showToast({
                            ...
                            success: function () {
                                ...
                                app.globalData['result'] = {rtype: 'success', rdesc: '操作成功', ndesc: '查看我的订单',
                                    ...
                                    links:[
                                        {link_desc:'我的订单',link:'../univ-list/univ-list?type=my&itemname=order'},
                                    ]
                                };
